...
def get_top(line_length):
    ...
    return _TAB_LENGTH + "".join(
        ["-" for _ in range(line_length + _TOP_BOTTOM_PADDING)]
    )
...
def get_padding(message, max_line_length):
    end_pad_length = max(
        [_PADDING_LENGTH, _PADDING_LENGTH + max_line_length - len(message)]
    )
    init_pad_length = min(
        [_PADDING_LENGTH, _PADDING_LENGTH + max_line_length - len(message)]
    )
...
def parse_message(message):
    ...
    for index, line in enumerate(wrapped_lines):
        ...
        formatted_message.append(
            "{}{} {} {}".format(
                "     ", delimiter[0], get_padding(line, max_line_length), delimiter[1]
            )
        )
